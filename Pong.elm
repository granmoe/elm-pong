...
stepBall t ({x,y,vx,vy} as ball) player1 player2 =
  if not (ball.x |> near 0 halfWidth)
    then { ball | x = 0, y = 0 }
      ...
      stepObj t
        { ball |
            vx = stepV vx (within ball player1) (within ball player2),
...
stepPlyr t dir points player =
  let player_ = stepObj t { player | vy = toFloat dir * 200 }
      y_      = clamp (22-halfHeight) (halfHeight-22) player_.y
      score_  = player.score + points
  in
...
stepGame input game =
  let
    ...
    player1_ = stepPlyr delta paddle1 score1 player1
    player2_ = stepPlyr delta paddle2 score2 player2
  in
      { game |
          state   = state_,
          ball    = ball_,
